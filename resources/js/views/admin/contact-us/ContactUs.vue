        ...
        <div class="card">
          ...
          <div class="card-body">
            ...
            <table-column :sortable="false" :filterable="false" label="User Name">
              ...
              <table-column show="message" label="Message"/>
              <table-column
                show="created_at"
                label="Registered On"
              ...
              >
                <template slot-scope="row">
                  <div class="table__actions">
                    <router-link v-if="row.is_replied != 1" :to="{ name: 'EmailReply', params: { contactId: row.id }}">
                      <a class="btn btn-default btn-sm">
                        <i class="icon-fa icon-fa-reply"/> Reply
                      </a>
                    </router-link>
...
export default {
  ...
  methods: {
    async getContactUsData ({ page, filter, sort }) {
      try {
        const response = await axios.get(`/api/admin/contact-us/get?page=${page}&filter=${filter}&sortcol=${sort.fieldName}&sort=${sort.order}`)
